...
def findNode(node_list, node_num):
	...
	Output: Returns a node object matching the node_num
	'''
	first = 0
	...
	while first <= last:
		...
		if node_list[midpoint].nodeID == node_num:
			...
			if node_num < node_list[midpoint].nodeID:
				...
				first = midpoint + 1	
	raise LookupError('findNode unable to find a node with id ' + str(node_num) + ' in node_list')
...
def lowestDegNode(node_list):
	...
	Output: a node object
	'''
	#Get array of lowest degree nodes
	lowest_deg = degreeOf(node_list[0])
	lowest_deg_nodes = []
	for node in node_list:
		if degreeOf(node) < lowest_deg:
			lowest_deg = degreeOf(node)
			lowest_deg_nodes = [node]
		elif degreeOf(node) == lowest_deg:
			lowest_deg_nodes.append(node)
		else:
			continue

	#Either return highest deg nodes or pick randomly
	if len(lowest_deg_nodes) == 1:
		return lowest_deg_nodes[0]
	else:
		return pickRandom(lowest_deg_nodes)
